...
export default function Sidebar({ activeItem, isCollapsed = false, onToggleCollapse, onCloseMobile }: SidebarProps) {
  ...
  return (
    <aside className={`h-screen flex flex-col justify-between border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#12161b] text-zinc-800 dark:text-zinc-200 transition-all duration-300 z-50 sticky top-0 ${isCollapsed ? 'w-20' : 'w-72'}`}>
        ...
        <Link href="/" className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} overflow-hidden select-none`}>
          ...
          {!isCollapsed && (
            <div className="flex flex-col animate-fade-in">
              ...
              <span className="text-[9px] font-medium text-zinc-400 dark:text-zinc-500 tracking-wider uppercase mt-1">Student Exchange</span>
            </div>
          )}
        </Link>
